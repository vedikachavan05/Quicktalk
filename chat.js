...
onMessage(messaging, (payload) => {
    ...
    const notificationOptions = {
        ...
        icon: 'images/quicktalk-logo.png' // Paths optimized for GitHub Pages compatibility
    };
...
// Event listener to change rooms via sidebar click
if (roomList) {
    roomList.addEventListener('click', (e) => {
        if (e.target.tagName === 'LI') {
            ...
            // Automatically close the sidebar on mobile layout after switching rooms
...
// Display available rooms from Firestore database dynamically
...
if (createRoomBtn) {
    createRoomBtn.addEventListener('click', async () => {
        ...
        if (roomName && roomName.trim() !== '') {
            ...
            try {
                await setDoc(doc(roomsCollection, roomName.trim()), {});
...
// Responsive Sidebar Toggle Execution
if (toggleSidebarBtn && roomsSidebar) {
    toggleSidebarBtn.addEventListener('click', () => {
        roomsSidebar.classList.toggle('active');
    });
}
